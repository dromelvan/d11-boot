...
CREATE OR REPLACE VIEW ${flyway:defaultSchema}.player_form_match_points AS
    SELECT player_id, season_id, SUM(points) form_points, ARRAY_TO_STRING(ARRAY_AGG(points),',') form_match_points FROM(
        SELECT * FROM(
            ...
            FROM ${flyway:defaultSchema}.player_match_stat pms
            JOIN ${flyway:defaultSchema}.match m ON pms.match_id = m.id
            JOIN ${flyway:defaultSchema}.match_week mw ON m.match_week_id = mw.id
        ) index_query
        WHERE index <= 5
...
CREATE OR REPLACE VIEW ${flyway:defaultSchema}.team_match_stats AS
SELECT *,
       matches_played home_matches_played,
       matches_won home_matches_won,
       matches_drawn home_matches_drawn,
       matches_lost home_matches_lost,
       goals_for home_goals_for,
       goals_against home_goals_against,
       goal_difference home_goal_difference,
       points home_points,
       0 away_matches_played,
       0 away_matches_won,
       0 away_matches_drawn,
       0 away_matches_lost,
       0 away_goals_for,
       0 away_goals_against,
       0 away_goal_difference,
       0 away_points
FROM(
    ...
    FROM(
        ...
        FROM ${flyway:defaultSchema}.match m
        JOIN ${flyway:defaultSchema}.match_week mw ON m.match_week_id = mw.id
    ) base_query
) sub_query
UNION
SELECT *,
       0 home_matches_played,
       0 home_matches_won,
       0 home_matches_drawn,
       0 home_matches_lost,
       0 home_goals_for,
       0 home_goals_against,
       0 home_goal_difference,
       0 home_points,
       matches_played away_matches_played,
       matches_won away_matches_won,
       matches_drawn away_matches_drawn,
       matches_lost away_matches_lost,
...
CREATE OR REPLACE VIEW ${flyway:defaultSchema}.team_form_match_points AS
    SELECT team_id, season_id, SUM(points) form_points, ARRAY_TO_STRING(ARRAY_AGG(points),',') form_match_points FROM(
        SELECT * FROM(
            SELECT tms.team_id, tms.match_week_id, tms.season_id, tms.points,
                   ROW_NUMBER() OVER (PARTITION BY team_id, season_id ORDER BY m.datetime DESC) AS index
            FROM ${flyway:defaultSchema}.team_match_stats tms
            JOIN ${flyway:defaultSchema}.match m ON tms.id = m.id
...
CREATE OR REPLACE VIEW ${flyway:defaultSchema}.d11_team_match_stats AS
SELECT *,
       matches_played home_matches_played,
       matches_won home_matches_won,
       matches_drawn home_matches_drawn,
...
FROM(
    ...
    FROM(
        ...
        FROM ${flyway:defaultSchema}.d11_match d11m
        JOIN ${flyway:defaultSchema}.match_week mw ON d11m.match_week_id = mw.id
    ) base_query
) sub_query
UNION
SELECT *,
...
CREATE OR REPLACE VIEW ${flyway:defaultSchema}.d11_team_form_match_points AS
    SELECT d11_team_id, season_id, SUM(points) form_points, ARRAY_TO_STRING(ARRAY_AGG(points),',') form_match_points FROM(
        SELECT * FROM(
            SELECT d11tms.d11_team_id, d11tms.match_week_id, d11tms.season_id, d11tms.points,
                   ROW_NUMBER() OVER (PARTITION BY d11_team_id, season_id ORDER BY d11m.datetime DESC) AS index
            FROM ${flyway:defaultSchema}.d11_team_match_stats d11tms
            JOIN ${flyway:defaultSchema}.d11_match d11m ON d11tms.id = d11m.id
...
CREATE OR REPLACE PROCEDURE ${flyway:defaultSchema}.update_stats_by_season_id(
...
BEGIN
    ...
    FROM (SELECT * FROM ${flyway:defaultSchema}.player_form_match_points WHERE season_id = update_season_id) pfmp
    ...
    SET matches_played = sub_query.matches_played,
        ...
        home_matches_played = sub_query.home_matches_played,
        home_matches_won = sub_query.home_matches_won,
        home_matches_drawn = sub_query.home_matches_drawn,
        home_matches_lost = sub_query.home_matches_lost,
        home_goals_for = sub_query.home_goals_for,
        home_goals_against = sub_query.home_goals_against,
        home_goal_difference = sub_query.home_goal_difference,
        home_points = sub_query.home_points,
        away_matches_played = sub_query.away_matches_played,
        away_matches_won = sub_query.away_matches_won,
        away_matches_drawn = sub_query.away_matches_drawn,
        away_matches_lost = sub_query.away_matches_lost,
        away_goals_for = sub_query.away_goals_for,
        away_goals_against = sub_query.away_goals_against,
        away_goal_difference = sub_query.away_goal_difference,
        away_points = sub_query.away_points
    FROM(
        SELECT team_id,
               season_id,
               COALESCE(SUM(matches_played),0) matches_played,
               COALESCE(SUM(matches_won),0) matches_won,
               COALESCE(SUM(matches_drawn),0) matches_drawn,
               COALESCE(SUM(matches_lost),0) matches_lost,
               COALESCE(SUM(goals_for),0) goals_for,
               COALESCE(SUM(goals_against),0) goals_against,
               COALESCE(SUM(goal_difference),0) goal_difference,
               COALESCE(SUM(points),0) points,
               COALESCE(SUM(home_matches_played),0) home_matches_played,
               COALESCE(SUM(home_matches_won),0) home_matches_won,
               COALESCE(SUM(home_matches_drawn),0) home_matches_drawn,
               COALESCE(SUM(home_matches_lost),0) home_matches_lost,
               COALESCE(SUM(home_goals_for),0) home_goals_for,
               COALESCE(SUM(home_goals_against),0) home_goals_against,
               COALESCE(SUM(home_goal_difference),0) home_goal_difference,
               COALESCE(SUM(home_points),0) home_points,
               COALESCE(SUM(away_matches_played),0) away_matches_played,
               COALESCE(SUM(away_matches_won),0) away_matches_won,
               COALESCE(SUM(away_matches_drawn),0) away_matches_drawn,
               COALESCE(SUM(away_matches_lost),0) away_matches_lost,
               COALESCE(SUM(away_goals_for),0) away_goals_for,
               COALESCE(SUM(away_goals_against),0) away_goals_against,
               COALESCE(SUM(away_goal_difference),0) away_goal_difference,
               COALESCE(SUM(away_points),0) away_points
        FROM ${flyway:defaultSchema}.team_match_stats
    ...
    FROM (SELECT * FROM ${flyway:defaultSchema}.team_form_match_points WHERE season_id = update_season_id) tfmp
    ...
    FROM(
        SELECT tmws.id team_match_week_stat_id,
               tmws.team_id,
               tmws.match_week_id,
               COALESCE(SUM(tms.matches_played),0) matches_played,
               COALESCE(SUM(tms.matches_won),0) matches_won,
               COALESCE(SUM(tms.matches_drawn),0) matches_drawn,
               COALESCE(SUM(tms.matches_lost),0) matches_lost,
               COALESCE(SUM(tms.goals_for),0) goals_for,
               COALESCE(SUM(tms.goals_against),0) goals_against,
               COALESCE(SUM(tms.goal_difference),0) goal_difference,
               COALESCE(SUM(tms.points),0) points,
               COALESCE(SUM(tms.home_matches_played),0) home_matches_played,
               COALESCE(SUM(tms.home_matches_won),0) home_matches_won,
               COALESCE(SUM(tms.home_matches_drawn),0) home_matches_drawn,
               COALESCE(SUM(tms.home_matches_lost),0) home_matches_lost,
               COALESCE(SUM(tms.home_goals_for),0) home_goals_for,
               COALESCE(SUM(tms.home_goals_against),0) home_goals_against,
               COALESCE(SUM(tms.home_goal_difference),0) home_goal_difference,
               COALESCE(SUM(tms.home_points),0) home_points,
               COALESCE(SUM(tms.away_matches_played),0) away_matches_played,
               COALESCE(SUM(tms.away_matches_won),0) away_matches_won,
               COALESCE(SUM(tms.away_matches_drawn),0) away_matches_drawn,
               COALESCE(SUM(tms.away_matches_lost),0) away_matches_lost,
               COALESCE(SUM(tms.away_goals_for),0) away_goals_for,
               COALESCE(SUM(tms.away_goals_against),0) away_goals_against,
               COALESCE(SUM(tms.away_goal_difference),0) away_goal_difference,
               COALESCE(SUM(tms.away_points),0) away_points
        FROM ${flyway:defaultSchema}.team_match_week_stat tmws
        JOIN ${flyway:defaultSchema}.team_match_stats tms ON tmws.team_id = tms.team_id AND tmws.match_week_id >= tms.match_week_id
        JOIN ${flyway:defaultSchema}.match_week mw ON tms.match_week_id = mw.id AND mw.season_id = update_season_id
        GROUP BY tmws.id
    ...
    FROM(
        ...
        FROM ${flyway:defaultSchema}.team_match_week_stat tmws
        JOIN ${flyway:defaultSchema}.team t ON tmws.team_id = t.id
        JOIN ${flyway:defaultSchema}.match_week mw ON tmws.match_week_id = mw.id
        WHERE mw.season_id = update_season_id
    ...
    FROM(
        SELECT d11_team_id,
               ...
               COALESCE(SUM(away_goal_difference),0) away_goal_difference,
               COALESCE(SUM(away_points),0) away_points
        FROM ${flyway:defaultSchema}.d11_team_match_stats
        WHERE season_id = update_season_id
        GROUP BY d11_team_id, season_id
    ...
    FROM (SELECT * FROM ${flyway:defaultSchema}.d11_team_form_match_points WHERE season_id = update_season_id) d11tfmp
    ...
    FROM(
        SELECT d11tmws.id d11_team_match_week_stat_id,
               ...
               COALESCE(SUM(d11tms.matches_played),0) matches_played,
               COALESCE(SUM(d11tms.matches_won),0) matches_won,
               COALESCE(SUM(d11tms.matches_drawn),0) matches_drawn,
               COALESCE(SUM(d11tms.matches_lost),0) matches_lost,
               COALESCE(SUM(d11tms.goals_for),0) goals_for,
               COALESCE(SUM(d11tms.goals_against),0) goals_against,
               COALESCE(SUM(d11tms.goal_difference),0) goal_difference,
               COALESCE(SUM(d11tms.points),0) points,
               COALESCE(SUM(d11tms.home_matches_played),0) home_matches_played,
               COALESCE(SUM(d11tms.home_matches_won),0) home_matches_won,
               COALESCE(SUM(d11tms.home_matches_drawn),0) home_matches_drawn,
               COALESCE(SUM(d11tms.home_matches_lost),0) home_matches_lost,
               COALESCE(SUM(d11tms.home_goals_for),0) home_goals_for,
               COALESCE(SUM(d11tms.home_goals_against),0) home_goals_against,
               COALESCE(SUM(d11tms.home_goal_difference),0) home_goal_difference,
               COALESCE(SUM(d11tms.home_points),0) home_points,
               COALESCE(SUM(d11tms.away_matches_played),0) away_matches_played,
               COALESCE(SUM(d11tms.away_matches_won),0) away_matches_won,
               COALESCE(SUM(d11tms.away_matches_drawn),0) away_matches_drawn,
               COALESCE(SUM(d11tms.away_matches_lost),0) away_matches_lost,
               COALESCE(SUM(d11tms.away_goals_for),0) away_goals_for,
               COALESCE(SUM(d11tms.away_goals_against),0) away_goals_against,
               COALESCE(SUM(d11tms.away_goal_difference),0) away_goal_difference,
               COALESCE(SUM(d11tms.away_points),0) away_points
        FROM ${flyway:defaultSchema}.d11_team_match_week_stat d11tmws
        JOIN ${flyway:defaultSchema}.d11_team_match_stats d11tms ON d11tmws.d11_team_id = d11tms.d11_team_id AND d11tmws.match_week_id >= d11tms.match_week_id
        JOIN ${flyway:defaultSchema}.match_week mw ON d11tms.match_week_id = mw.id AND mw.season_id = update_season_id
        GROUP BY d11tmws.id
    ...
    FROM(
        ...
        FROM ${flyway:defaultSchema}.d11_team_match_week_stat d11tmws
        JOIN ${flyway:defaultSchema}.d11_team d11t ON d11tmws.d11_team_id = d11t.id
        JOIN ${flyway:defaultSchema}.match_week mw ON d11tmws.match_week_id = mw.id
        WHERE mw.season_id = update_season_id
    ...
    ) WHERE ${flyway:defaultSchema}.match_week.season_id = update_season_id;
    ...
    SET most_valuable_player_id = (
        ...
        AND ${flyway:defaultSchema}.match_week.status IN (1,2,3);
