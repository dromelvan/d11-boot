 ...
 * require.config.params['matchheader'] = {
 * input: [189,13,'Brentford','Arsenal','13/08/2021 20:00:00','13/08/2021 00:00:00',6,'FT','1 : 2','1 : 2',,,'1 : 2',
 * 'England','England'],
 * matchId: 1549539
 * };
 */
...
public class MatchHeader {
     ...
     */
    public static final Pattern MATCH_HEADER_PATTERN =
            Pattern.compile(".*require.config.params\\['matchheader'] = (\\{.*});.*", Pattern.DOTALL);
    ...
    public Long getHomeTeamId() {
        if (this.input != null) {
            return ((Integer) this.input.get(HOME_TEAM_ID_INDEX)).longValue();
        }
        return null;
    }
    ...
    public Long getAwayTeamId() {
        if (this.input != null) {
            return ((Integer) this.input.get(AWAY_TEAM_ID_INDEX)).longValue();
        }
        return null;
    }
    ...
    public LocalDateTime getDatetime() {
        if (this.input != null) {
            final LocalDateTime localDateTime = LocalDateTime.parse((String) this.input.get(DATE_TIME_INDEX),
                                                                    DATE_TIME_FORMATTER);
    ...
    public Status getStatus() {
        return switch (getElapsed()) {
            case ELAPSED_PENDING -> Status.PENDING;
            case ELAPSED_FULL_TIME -> Status.FULL_TIME;
            case ELAPSED_POSTPONED -> Status.POSTPONED;
     ...
     */
    public ParsedMatchData getMatchData() {
        final ParsedMatchData matchData = new ParsedMatchData();
        matchData.setSiteId(getMatchId());
        matchData.setHomeTeamName(getHomeTeamName());
        matchData.setHomeTeamSiteId(getHomeTeamId());
        matchData.setAwayTeamName(getAwayTeamName());
        matchData.setAwayTeamSiteId(getAwayTeamId());
