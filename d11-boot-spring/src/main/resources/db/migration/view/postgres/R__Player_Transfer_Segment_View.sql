-- Drop in reverse dependency order so CREATE OR REPLACE works cleanly.
DROP MATERIALIZED VIEW IF EXISTS player_transfer_segment CASCADE;
DROP VIEW IF EXISTS player_match_week_points CASCADE;
DROP VIEW IF EXISTS player_match_week_fee_segment CASCADE;
DROP VIEW IF EXISTS player_match_week_fee CASCADE;
...
CREATE MATERIALIZED VIEW player_transfer_segment AS
...
CREATE INDEX IF NOT EXISTS idx_player_transfer_segment_player_id
    ON player_transfer_segment (player_id);

CREATE INDEX IF NOT EXISTS idx_player_transfer_segment_season_id
    ON player_transfer_segment (season_id);

CREATE INDEX IF NOT EXISTS idx_player_transfer_segment_d11_team_id
    ON player_transfer_segment (d11_team_id);

CREATE INDEX IF NOT EXISTS idx_player_transfer_segment_player_season
    ON player_transfer_segment (player_id, season_id);
